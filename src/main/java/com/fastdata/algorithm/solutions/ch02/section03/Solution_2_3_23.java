 ...
 * @Author: Lucky
 * @Description:
 * @Date: created in 2021/5/9 - 13:46
 */
public class Solution_2_3_23 {
    ...
    private static void doExperiment(int numberOfExperiments, int initialArraySize, Map<Integer, Comparable[]> allInputArrays) {
        ...
        for(int i = 0; i < numberOfExperiments; i++) {
            ...
            Solution_2_3_22.quickSortWithFast3WayPartitioning(arrayCopy1);
    ...
    private static int getMedianIndex(Comparable[] array, int index1, int index2, int index3) {
        return ArrayUtil.less(array[index1], array[index2]) ? (ArrayUtil.less(array[index2], array[index3]) ? index2: ArrayUtil.less(array[index1], array[index3])? index3: index1) :
                (ArrayUtil.less(index1, index3) ? index1: ArrayUtil.less(index2, index3) ? index3: index2);
    }

    private static void printResults(int arraySize, double quickSort3WayRunningTime, double quickSortWithFast3WayPartioningRunningTime, double quickSortWithFast3WayPartioningTukeyNintherRunningTime) {
        StdOut.printf("%10d %18.1f %40.1f %55.1f\n", arraySize, quickSort3WayRunningTime, quickSortWithFast3WayPartioningRunningTime, quickSortWithFast3WayPartioningTukeyNintherRunningTime);
    }
}
