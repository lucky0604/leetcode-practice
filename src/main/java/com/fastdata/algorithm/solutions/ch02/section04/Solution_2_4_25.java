 ...
 * @Date: 2021-06-05 11:50 AM
 * @Version: 1.0
 * @description:
 **/

public class Solution_2_4_25 {
    ...
    private void insertNextElement(PriorityQueue<ValueForComputation> priorityQueue, ValueForComputation value, int maxJValue) {
        ...
        BigInteger newComputedValue = iValue.multiply(iValue).multiply(iValue).add(jValue.multiply(jValue).multiply(jValue));
        ValueForComputation newValue = new ValueForComputation(newComputedValue, value.i, value.j + 1);
        priorityQueue.insert(newValue);
    }

    private void getEqualValues(PriorityQueue<ValueForComputation> priorityQueue, ValueForComputation currentValue, List<ValueForComputation> equalResults, int n) {
